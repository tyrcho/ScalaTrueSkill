...
public class GaussianDistributionTests {
	...
	public void CumulativeToTests() {
		...
		// )
		assertEquals(0.691462, GaussianDistribution.cumulativeTo(0.5),
				ErrorTolerance);
	}
	...
	public void MultiplicationTests() {
		...
		GaussianDistribution product = GaussianDistribution.prod(
		...
		GaussianDistribution product2 = GaussianDistribution.prod(m4s5, m6s7);

		double expectedMean = (4 * square(7) + 6 * square(5))
				/ (square(5) + square(7));
	...
	public void DivisionTests() {
		...
		GaussianDistribution product = new GaussianDistribution(0.2,
				3.0 / sqrt(10));
		GaussianDistribution standardNormal = new GaussianDistribution(0, 1);

		GaussianDistribution productDividedByStandardNormal = GaussianDistribution
				.divide(product, standardNormal);
		...
		GaussianDistribution product2 = new GaussianDistribution(
				(4 * square(7) + 6 * square(5)) / (square(5) + square(7)),
				sqrt(((square(5) * square(7)) / (square(5) + square(7)))));
		GaussianDistribution m4s5 = new GaussianDistribution(4, 5);
		GaussianDistribution product2DividedByM4S5 = GaussianDistribution
				.divide(product2, m4s5);
	...
	public void LogProductNormalizationTests() {
		...
		GaussianDistribution standardNormal = new GaussianDistribution(0, 1);
		double lpn = GaussianDistribution.logProductNormalization(
	...
	public void AbsoluteDifferenceTests() {
		// Verified with Ralf Herbrich's F# implementation
		GaussianDistribution standardNormal = new GaussianDistribution(0, 1);
		double absDiff = GaussianDistribution.absoluteDifference(
				standardNormal, standardNormal);
