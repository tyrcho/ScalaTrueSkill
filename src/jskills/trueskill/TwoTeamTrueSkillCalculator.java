...
public class TwoTeamTrueSkillCalculator extends SkillCalculator {
	...
	private static void UpdatePlayerRatings(GameInfo gameInfo,
			Map<IPlayer, Rating> newPlayerRatings, ITeam selfTeam,
			ITeam otherTeam, PairwiseComparison selfToOtherTeamComparison) {
		...
		for (Entry<IPlayer, Rating> teamPlayerRatingPair : selfTeam.entrySet()) {
			...
			double meanMultiplier = (square(previousPlayerRating
					.getStandardDeviation()) + tauSquared) / c;
			double stdDevMultiplier = (square(previousPlayerRating
					.getStandardDeviation()) + tauSquared) / square(c);

			double playerMeanDelta = (rankMultiplier * meanMultiplier * v);
			double newMean = previousPlayerRating.getMean() + playerMeanDelta;

			double newStdDev = Math.sqrt((square(previousPlayerRating
