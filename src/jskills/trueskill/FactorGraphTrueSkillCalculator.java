...
public class FactorGraphTrueSkillCalculator extends SkillCalculator {
	public FactorGraphTrueSkillCalculator() {
		super(EnumSet.of(SupportedOptions.PartialPlay,
				SupportedOptions.PartialUpdate), Range.atLeast(2), Range
				.atLeast(1));
	}
	...
	public Map<IPlayer, Rating> calculateNewRatings(GameInfo gameInfo,
		...
		TrueSkillFactorGraph factorGraph = new TrueSkillFactorGraph(gameInfo,
				teamsl, teamRanks);
	...
	public double calculateMatchQuality(GameInfo gameInfo,
		...
		SimpleMatrix playerTeamAssignmentsMatrix = CreatePlayerTeamAssignmentMatrix(
				teamAssignmentsList, meanVector.numRows());
		SimpleMatrix playerTeamAssignmentsMatrixTranspose = playerTeamAssignmentsMatrix
				.transpose();

		double betaSquared = square(gameInfo.getBeta());

		SimpleMatrix start = meanVectorTranspose
				.mult(playerTeamAssignmentsMatrix);
		SimpleMatrix aTa = playerTeamAssignmentsMatrixTranspose.mult(
				playerTeamAssignmentsMatrix).scale(betaSquared);
		SimpleMatrix aTSA = playerTeamAssignmentsMatrixTranspose.mult(
				skillsMatrix).mult(playerTeamAssignmentsMatrix);
		SimpleMatrix middle = aTa.plus(aTSA);

		SimpleMatrix middleInverse = middle.invert();

		SimpleMatrix end = playerTeamAssignmentsMatrixTranspose
				.mult(meanVector);

		SimpleMatrix expPartMatrix = start.mult(middleInverse).mult(end)
				.scale(-0.5);
	...
	private static SimpleMatrix CreatePlayerTeamAssignmentMatrix(
		...
		for (int i = 0; i < teamAssignmentsList.size() - 1; i++) {
			...
			for (IPlayer player : currentTeam.keySet()) {
				currentRowValues.add(PartialPlay
						.getPartialPlayPercentage(player));
			...
			for (IPlayer nextTeamPlayer : nextTeam.keySet()) {
				...
				currentRowValues.add(-1
						* PartialPlay.getPartialPlayPercentage(nextTeamPlayer));
			}
		}

		SimpleMatrix playerTeamAssignmentsMatrix = new SimpleMatrix(
				totalPlayers, teamAssignmentsList.size() - 1);
		for (int i = 0; i < playerAssignments.size(); i++)
			for (int j = 0; j < playerAssignments.get(i).size(); j++)
				playerTeamAssignmentsMatrix.set(j, i, playerAssignments.get(i)
						.get(j));
