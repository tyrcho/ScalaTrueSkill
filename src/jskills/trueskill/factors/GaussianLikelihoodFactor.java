...
public class GaussianLikelihoodFactor extends GaussianFactor {
	...
	public GaussianLikelihoodFactor(double betaSquared,
		...
		super(String.format("Likelihood of %s going to %s", variable2,
				variable1));
	...
	public double getLogNormalization() {
		return logRatioNormalization(variables.get(0).getValue(),
				messages.get(0).getValue());
	}
	...
	private double UpdateHelper(Message<GaussianDistribution> message1,
			...
			Variable<GaussianDistribution> variable1,
			Variable<GaussianDistribution> variable2) {
		GaussianDistribution message1Value = new GaussianDistribution(
				message1.getValue());
		GaussianDistribution message2Value = new GaussianDistribution(
				message2.getValue());

		GaussianDistribution marginal1 = new GaussianDistribution(
				variable1.getValue());
		GaussianDistribution marginal2 = new GaussianDistribution(
				variable2.getValue());

		double a = _Precision
				/ (_Precision + marginal2.getPrecision() - message2Value
						.getPrecision());

		GaussianDistribution newMessage = GaussianDistribution
				.fromPrecisionMean(
						a
								* (marginal2.getPrecisionMean() - message2Value
										.getPrecisionMean()),
						a
								* (marginal2.getPrecision() - message2Value
										.getPrecision()));

		GaussianDistribution oldMarginalWithoutMessage = divide(marginal1,
				message1Value);

		GaussianDistribution newMarginal = prod(oldMarginalWithoutMessage,
				newMessage);
