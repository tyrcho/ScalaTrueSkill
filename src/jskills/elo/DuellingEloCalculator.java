...
public class DuellingEloCalculator extends SkillCalculator {
	...
	public DuellingEloCalculator(TwoPlayerEloCalculator twoPlayerEloCalculator) {
		super(EnumSet.noneOf(SupportedOptions.class), Range.atLeast(2), Range
				.atLeast(1));
	...
	public Map<IPlayer, Rating> calculateNewRatings(GameInfo gameInfo,
		...
		for (int ixCurrentTeam = 0; ixCurrentTeam < teamsList.length; ixCurrentTeam++) {
			for (int ixOtherTeam = 0; ixOtherTeam < teamsList.length; ixOtherTeam++) {
				...
				PairwiseComparison comparison = PairwiseComparison
						.fromMultiplier((int) Math
								.signum(teamRanks[ixOtherTeam]
										- teamRanks[ixCurrentTeam]));

				for (Entry<IPlayer, Rating> currentTeamPlayerRatingPair : currentTeam
						.entrySet()) {
					for (Entry<IPlayer, Rating> otherTeamPlayerRatingPair : otherTeam
							.entrySet()) {
		...
		for (ITeam currentTeam : teamsList) {
			for (Entry<IPlayer, Rating> currentTeamPlayerPair : currentTeam
					.entrySet()) {
				double currentPlayerAverageDuellingDelta = MathUtils
						.mean(deltas.get(currentTeamPlayerPair.getKey())
								.values());
	...
	private void updateDuels(GameInfo gameInfo,
		...
		Map<IPlayer, Rating> duelOutcomes = twoPlayerEloCalc
				.calculateNewRatings(
						gameInfo,
						Team.concat(new Team(player1, player1Rating), new Team(
								player2, player2Rating)),
						(weakToStrongComparison == PairwiseComparison.WIN) ? new int[] {
								1, 2 }
								: (weakToStrongComparison == PairwiseComparison.LOSE) ? new int[] {
										2, 1 }
										: new int[] { 1, 1 });
	...
	public double calculateMatchQuality(GameInfo gameInfo,
		...
		for (int ixCurrentTeam = 0; ixCurrentTeam < teamList.length; ixCurrentTeam++) {
			EloRating currentTeamAverageRating = new EloRating(
					Rating.calcMeanMean(teamList[ixCurrentTeam].values()));
			;
			Team currentTeam = new Team(new Player<Integer>(ixCurrentTeam),
					currentTeamAverageRating);

			for (int ixOtherTeam = ixCurrentTeam + 1; ixOtherTeam < teamList.length; ixOtherTeam++) {
				EloRating otherTeamAverageRating = new EloRating(
						Rating.calcMeanMean(teamList[ixOtherTeam].values()));
				Team otherTeam = new Team(new Player<Integer>(ixOtherTeam),
